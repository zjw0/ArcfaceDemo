package com.arcsoft.arcfacedemo.common;

public class Constants {

//    public static final String APP_ID = "官网获取的APP_ID";
//    public static final String SDK_KEY = "官网获取的SDK_KEY";
    public static final String APP_ID = "D8bjoSFLmZkFxGnSo1bY874hADouHae6rEnaHAQasQwn";
    public static final String SDK_KEY = "HtpNA2XTBsisAhXQpiVLYNcTSUc6FNekQNZTo4BAeRQX";

    /**
     * IR预览数据相对于RGB预览数据的横向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int HORIZONTAL_OFFSET = 0;
    /**
     * IR预览数据相对于RGB预览数据的纵向偏移量，注意：是预览数据，一般的摄像头的预览数据都是 width > height
     */
    public static final int VERTICAL_OFFSET = 0;
}

